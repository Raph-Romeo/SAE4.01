...
class Withdrawal(Transaction):
    ...
    def serialize(self, user):
        if self.name is None:
            ...
            name = [True, self.name]
        return {"id":self.id,"name": name, "is_pending": self.is_pending, "amount": self.amount_str(), "type": 2, "transaction_date": self.transaction_date, "from": [str(self.source), str(self.source.client)], "to": [str(self.destination)]}
