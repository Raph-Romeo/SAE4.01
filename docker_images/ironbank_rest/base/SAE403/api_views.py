...
def publish_to_nats(sender, **kwargs):
    ...
    async def run():
        await nc.connect(servers=['nats://172.16.1.30:4222'])
            ...
            if request.data.get("type") == 1:
                ...
                if Account.objects.get(pk=destination).client == request.user:
                    if amount >= 10000:
                        ...
                        data = '{"account_id":' + str(destination) + ',"amount":' + str(amount) + "}"
                        nats_publish_signal.send(sender=None, message=data, subject='ironbank/deposit')
                ...
                if Account.objects.get(pk=source).client == request.user:
                    if amount >= 1000:
                        transaction = Withdrawal(name=name, is_pending=True, amount=amount, source=Account.objects.get(pk=source), destination=request.user)
                        transaction.save()
                    else:
                        transaction = Withdrawal(name=name, is_pending=False, amount=amount, source=Account.objects.get(pk=source), destination=request.user)
                        transaction.save()

                        nats_publish_signal = Signal()
                        nats_publish_signal.connect(publish_to_nats)
                        data = '{"account_id":' + str(source) + ',"amount":' + str(amount) + "}"
                        nats_publish_signal.send(sender=None, message=data, subject='ironbank/withdrawal')
                ...
                if request.user == Account.objects.get(pk=source).client:
                    ...
                    if Account.objects.get(pk=source) != send_to:
                        if amount >= 10000:
                            ...
                            nats_publish_signal = Signal()
                            nats_publish_signal.connect(publish_to_nats)
                            data = '{"account_id_source":' + str(source) + ',"account_id_destination":' + str(send_to.id) + ',"amount":' + str(amount) + "}"
                            nats_publish_signal.send(sender=None, message=data, subject='ironbank/transfer')
        ...
        if request.user.is_staff:
            if type == 1:
                ...
                if request.data.get("authorize"):
                    ...
                    nats_publish_signal = Signal()
                    nats_publish_signal.connect(publish_to_nats)
                    data = '{"account_id":' + str(transaction.destination.id) + ',"amount":' + str(transaction.amount) + '}'
                    nats_publish_signal.send(sender=None, message=data, subject='ironbank/deposit')
                    return Response(transaction.serialize(request.user))
                else:
                    transaction.delete()
                ...
                if request.data.get("authorize"):
                    ...
                    nats_publish_signal = Signal()
                    nats_publish_signal.connect(publish_to_nats)
                    data = '{"account_id":' + str(transaction.source.id) + ',"amount":' + str(transaction.amount) + '}'
                    nats_publish_signal.send(sender=None, message=data, subject='ironbank/withdrawal')
                    return Response(transaction.serialize(request.user))
                else:
                    transaction.delete()
                ...
                if request.data.get("authorize"):
                    ...
                    nats_publish_signal = Signal()
                    nats_publish_signal.connect(publish_to_nats)
                    data = '{"account_id_source":' + str(transaction.source.id) + ',"account_id_destination":' + str(transaction.destination.id) + ',"amount":' + str(transaction.amount) + '}'
                    nats_publish_signal.send(sender=None, message=data, subject='ironbank/transfer')
                    return Response(transaction.serialize(request.user))
                else:
                    transaction.delete()
